...
int s2n_negotiate(struct s2n_connection *conn, s2n_blocked_status *blocked)
{
    ...
    while (ACTIVE_STATE(conn).writer != 'B') {
        ...
        if (conn->mode == S2N_CLIENT && ACTIVE_MESSAGE(conn) == CLIENT_HELLO) {
        ...
        if (conn->mode == S2N_CLIENT && (ACTIVE_MESSAGE(conn) == SERVER_FINISHED || ACTIVE_MESSAGE(conn) == APPLICATION_DATA)) {
